<nav class="sidebar sidebar-offcanvas" id="sidebar">
        <ul class="nav">
          <li class="nav-item">
            <a class="nav-link" href='tableau-de-bord'>
              <i class="menu-icon mdi mdi-floor-plan"></i>
              <span class="menu-title">Tableau de bord</span>
            </a>
          </li>
      
         

          <li class="nav-item">
            <a class="nav-link"  href="clients"  >
              <i class="menu-icon mdi mdi-account-multiple-outline "></i>
              <span class="menu-title">Clients</span>
               
            </a>
           
          </li>
          
          <li class="nav-item">
            <a class="nav-link"  href="réservations" aria-expanded="false" aria-controls="form-elements">
              <i class="menu-icon mdi mdi-clipboard-check "></i>
              <span class="menu-title">Réservations</span>
              
            </a>
            
          </li>
          <li class="nav-item">
            <a class="nav-link"  href="parkings" aria-expanded="false" aria-controls="charts">
              <i class="menu-icon  mdi mdi-parking "></i>
              <span class="menu-title">Parkings</span>
              
            </a>
           
          </li>
          <li class="nav-item">
            <a class="nav-link"  href="véhicules" aria-expanded="false" aria-controls="tables">
              <i class="menu-icon  mdi mdi-car "></i>
              <span class="menu-title">Véhicules</span>
              
            </a>
            
          </li>
          <li class="nav-item">
            <a class="nav-link"  href="codespromo" aria-expanded="false" aria-controls="icons">
              <i class="menu-icon  mdi mdi-sale "></i>
              <span class="menu-title">Codes promos</span>
              
            </a>
            
          </li>
          
         
          </li>
          
          
        </ul>
      </nav>